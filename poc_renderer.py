...
# Embedded 8x16 VGA font data centered into 10x16 cells
FONT_DATA = [
...
def wrap_text(text, max_chars=80, tab_width=4):
    """Word wraps lines exceeding max_chars, expanding tabs."""
    ...
    for line in text.splitlines():
        ...
        while len(line) > 0:
            ...
            segment = line[:wrap_idx]
            wrapped.append(segment)
            max_line_len = max(max_line_len, len(segment))

            line = line[wrap_idx:]
            if len(line) > 0 and line[0] == ' ':
                line = line[1:]

    return wrapped, max_line_len
...
def render_text_to_images(
    ...
    using the embedded custom 10x16 bitmap font engine.
    """
    # 1. Word wrap the input text
    lines, max_line_len = wrap_text(text, max_chars=80, tab_width=tab_width)
    if not lines:
        lines = [""]
    ...
    print(f"Total lines: {len(lines)}. Splitting into {len(pages)} page(s).")

    generated_files = []

    # 3. Render pages
    for page_idx, page_lines in enumerate(pages, 1):
        page_max_len = max(len(line) for line in page_lines) if page_lines else 0

        # Calculate dimensions (10x16 per character cell + margins)
        img_width = (page_max_len * 10) + (margin * 2)
        img_height = (len(page_lines) * 16) + (margin * 2)
        ...
        for line_idx, line_text in enumerate(page_lines):
            y_cell = margin + (line_idx * 16)
            for char_idx, char in enumerate(line_text):
                x_cell = margin + (char_idx * 10)
                code = ord(char)
                if code < 0 or code >= 128:
                    code = 63 # '?'

                glyph = FONT_DATA[code]
                for r in range(16):
                    row_val = glyph[r]
                    for c in range(8):
                        pixel_on = (row_val >> (7 - c)) & 1
                        if pixel_on:
                            # Centered 8px width inside 10px cell
                            pixels[x_cell + 1 + c, y_cell + r] = 0
